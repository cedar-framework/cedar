     ...
     &                ) BIND(C, NAME='BMG2_SymStd_SOLVE_cg')
...
!
!   spbsl and sposl are linpack routines.
!
      IF(IPN.NE.PER_y .AND. IPN.NE.PER_x .AND. IPN.NE.PER_xy) THEN
         CALL DPBTRS('U',KK,I1,1,ABD,NABD1,BBD,NABD2,INFO)
!         CALL DPBSL(ABD,NABD1,N,I1,BBD)
      ELSE
         CALL DPOTRS('U',KK,1,ABD,NABD1,BBD,NABD2,INFO)
!         CALL DPOSL(ABD,NABD1,N,BBD)
      ENDIF
