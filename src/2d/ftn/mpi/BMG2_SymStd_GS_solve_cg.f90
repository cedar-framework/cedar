SUBROUTINE copy_so_to_abd(so, abd, ii, jj, nstncl)
  use ModInterface
  implicit none

  integer(len_t), value :: ii, jj
  integer(c_int), value :: nstncl
  real(real_t) :: so(ii+1, jj+1, nstncl), abd(ii+1, jj+1, nstncl)
...
end SUBROUTINE copy_so_to_abd

SUBROUTINE BMG2_SymStd_GS_solve_cg ( &
     &                K, SO, QF, Q, II, JJ, &
     &                KF, IFD, NStncl, IRELAX_SYM, UPDOWN,&
     &                iGs, jGs, halof&
     &                ) BIND(C, NAME='MPI_BMG2_SymStd_GS_solve_cg')
