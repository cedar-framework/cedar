     ...
     &                K, SO, QF, Q, SOR, II, JJ, nplanes,&
     &                KF, IFD, NStncl, IRELAX_SYM, UPDOWN,&
     &                iGs, jGs, halof&
     &                ) BIND(C, NAME='MPI_BMG2_SymStd_relax_GS_plane')
...
!
      ...
      INTEGER(C_INT), VALUE :: NStncl, nplanes

      REAL(real_t) :: Q(II,JJ,nplanes), QF(II,JJ), SO(II+1,JJ+1,NStncl), SOR(II,JJ,*)
...
!     Local Declarations
!
      INTEGER I, I1, I2, IBEG, J, J1, J2,p
      ...
      IF ( K.LT.KF .OR. IFD.NE.1 ) THEN
         ...
         DO pts=ptstart,ptend,ptstride
            do p=1,nplanes
               DO  J=JB(pts),J1,2
                  DO  I=IB(pts),I1,2
                     Q(I,J,p)=(QF(I,J)+SO(I,J,KW)*Q(I-1,J,p)+SO(I+1,J,KW)&
                          &                 *Q(I+1,J,p)+SO(I,J,KS)*Q(I,J-1,p)+SO(I,J+1,KS)&
                          &                 *Q(I,J+1,p)+SO(I,J,KSW)*Q(I-1,J-1,p)&
                          &                 +SO(I+1,J,KNW)*Q(I+1,J-1,p)+SO(I,J+1,KNW)&
                          &                 *Q(I-1,J+1,p)+SO(I+1,J+1,KSW)*Q(I+1,J+1,p))&
                          &                 *SOR(I,J,MSOR)
