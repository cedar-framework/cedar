                       ...
                       std::vector<cedar::real_t> & x1)
{
	...
	for (std::size_t i = 0; i < x0.size(); i++) {
		if (std::abs(x0[i] - x1[i]) >= tol) {
			std::cout << "HERE" << std::endl;
			std::cout << "[" << i << "] "
			          << x0[i] << " vs " << x1[i] << std::endl;
		}
...
TEST(MPIPlaneAgg, LineRelax5) {
	...
	auto ynoagg = line_agg<relax_dir::y, five_pt>(false, 29, 28, 2);
	auto yagg   = line_agg<relax_dir::y, five_pt>(true,  29, 28, 2);
