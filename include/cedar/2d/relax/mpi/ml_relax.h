...
public:
	...
	void init_ndim(int nproc, int nproc_other, int coord, len_t nlines, len_t nlines_other,
	               int min_gsz, int nog, MPI_Comm comm)
	{
			...
			MPI_Comm gcomm;
			MPI_Comm_split(comm, nproc_other, nproc,
			               &gcomm);
			comms(0, this->nlevels - 2) = MPI_Comm_c2f(gcomm);
			MPI_BMG2_SymStd_SETUP_comm(comms.data(), this->nlevels, coord, min_gsz);
		           ...
		           relax_stencil & sor)
	{
		...
		if (dir == relax_dir::x) {
			init_ndim(topo.nproc(0), topo.nproc(1), topo.coord(0),
			...
			init_ndim(topo.nproc(1), topo.nproc(0), topo.coord(1),
